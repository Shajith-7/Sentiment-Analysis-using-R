...
library(sentimentr)
                ...
                #import dataset and basic info about that..........
...
data <- read.csv("Rproject.csv", stringsAsFactors = FALSE)
  ...
  column_names <- colnames(data)  # Names of the columns
...
data_without_last_two <- data[, -c(ncol(data)-1, ncol(data))]
 ...
 if (length(positive_words_in_text) > 0) {
   wordcloud(positive_words_in_text, max.words = 100, scale = c(5, 0.5), colors = c("red", "black"), main = "Positive Words")
 } else {
